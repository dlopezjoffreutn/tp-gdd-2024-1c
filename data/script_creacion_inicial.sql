...
BEGIN
    INSERT INTO DASE_DE_BATOS.VENTAS (NUMERO, FECHA_HORA, CAJA_ID, EMPLEADO_ID, TIPO_COMPROBANTE_ID)
    SELECT DISTINCT
        m.TICKET_NUMERO,
        m.TICKET_FECHA_HORA,
        c.ID AS CAJA_ID,
...
CREATE PROCEDURE DASE_DE_BATOS.SP_DETALLE_PAGO_TARJETA AS
	BEGIN
	INSERT INTO DASE_DE_BATOS.DETALLE_PAGO_TARJETA (PAGO_ID, CLIENTE_ID, NRO_TARJETA, FECHA_VENCIMIENTO_TARJETA, CUOTAS)
		SELECT DISTINCT
		(
			SELECT TOP 1 p.ID
			...
			JOIN DASE_DE_BATOS.CAJAS c ON c.ID = v.CAJA_ID
			JOIN DASE_DE_BATOS.SUCURSALES s ON s.ID = c.SUCURSAL_ID
			WHERE p.FECHA_HORA = m.PAGO_FECHA AND p.IMPORTE = m.PAGO_IMPORTE AND p.VENTA_ID = v.ID AND c.NUMERO = m.CAJA_NUMERO AND s.NOMBRE = m.SUCURSAL_NOMBRE
		),
		(
			SELECT TOP 1 c.ID
		...
		FROM gd_esquema.Maestra m
		WHERE m.PAGO_FECHA IS NOT NULL AND m.PAGO_IMPORTE IS NOT NULL AND m.TICKET_NUMERO IS NOT NULL AND m.PAGO_TIPO_MEDIO_PAGO IS NOT NULL AND m.PAGO_TARJETA_NRO IS NOT NULL AND m.PAGO_TARJETA_FECHA_VENC IS NOT NULL AND m.PAGO_TARJETA_CUOTAS IS NOT NULL
		AND m.CLIENTE_DNI IS NOT NULL AND m.CLIENTE_NOMBRE IS NOT NULL AND m.CLIENTE_APELLIDO IS NOT NULL
	END
GO
...
CREATE PROCEDURE DASE_DE_BATOS.SP_PAGO_DESCUENTO AS
    ...
    INSERT INTO DASE_DE_BATOS.PAGO_DESCUENTO (DESCUENTO_ID, PAGO_ID)
		...
		(
			...
			JOIN DASE_DE_BATOS.CAJAS c ON c.ID = v.CAJA_ID
			JOIN DASE_DE_BATOS.SUCURSALES s ON s.ID = c.SUCURSAL_ID
			WHERE p.FECHA_HORA = m.PAGO_FECHA AND p.IMPORTE = m.PAGO_IMPORTE AND v.NUMERO = m.TICKET_NUMERO AND c.NUMERO = m.CAJA_NUMERO AND s.NOMBRE = m.SUCURSAL_NOMBRE
		)
		FROM gd_esquema.Maestra m
		WHERE m.DESCUENTO_DESCRIPCION IS NOT NULL AND m.DESCUENTO_FECHA_INICIO IS NOT NULL AND m.DESCUENTO_FECHA_FIN IS NOT NULL AND m.DESCUENTO_PORCENTAJE_DESC IS NOT NULL AND m.DESCUENTO_TOPE IS NOT NULL
		AND m.PAGO_FECHA IS NOT NULL AND m.PAGO_IMPORTE IS NOT NULL AND m.TICKET_NUMERO IS NOT NULL AND m.PAGO_MEDIO_PAGO IS NOT NULL
	END
GO
----
