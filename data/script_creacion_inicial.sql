...
---- Create indice ----
CREATE INDEX IDX_LOCALIDADES_PROVINCIA_ID ON DASE_DE_BATOS.LOCALIDADES(PROVINCIA_ID);
----
...
---- Create indice ----
CREATE INDEX IDX_SUCURSALES_LOCALIDAD_ID ON DASE_DE_BATOS.SUCURSALES(LOCALIDAD_ID);
----
...
CREATE TABLE DASE_DE_BATOS.CAJAS (
	ID decimal(18, 0) IDENTITY PRIMARY KEY,
	NUMERO decimal(18, 0), -- m.CAJA_NUMERO
...
---- Create indice ----
CREATE INDEX IDX_CAJAS_TIPO_ID ON DASE_DE_BATOS.CAJAS(TIPO_ID);
----
...
---- Create indice ----
CREATE INDEX IDX_EMPLEADOS_SUCURSAL_ID ON DASE_DE_BATOS.EMPLEADOS(SUCURSAL_ID);
----
...
CREATE TABLE DASE_DE_BATOS.VENTAS (
	ID decimal(18, 0) IDENTITY PRIMARY KEY,
	NUMERO decimal(18, 0), -- m.TICKET_NUMERO
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        p.ID
    FROM gd_esquema.Maestra m
    JOIN DASE_DE_BATOS.PROVINCIAS p ON p.NOMBRE = m.CLIENTE_PROVINCIA
    ...
    SELECT DISTINCT
        ...
        p.ID
    FROM gd_esquema.Maestra m
    JOIN DASE_DE_BATOS.PROVINCIAS p ON p.NOMBRE = m.SUCURSAL_PROVINCIA
    ...
    SELECT DISTINCT
        ...
        p.ID
    FROM gd_esquema.Maestra m
    JOIN DASE_DE_BATOS.PROVINCIAS p ON p.NOMBRE = m.SUPER_PROVINCIA
...
BEGIN
    INSERT INTO DASE_DE_BATOS.VENTAS (NUMERO, FECHA_HORA, CAJA_ID, EMPLEADO_ID, TIPO_COMPROBANTE_ID, SUBTOTAL, TOTAL_DESCUENTOS, TOTAL_PROMOCIONES, TOTAL)
    SELECT DISTINCT
        m.TICKET_NUMERO,
        m.TICKET_FECHA_HORA,
        c.ID AS CAJA_ID,
    ...
    FROM gd_esquema.Maestra m
    JOIN DASE_DE_BATOS.CAJAS c ON c.NUMERO = m.CAJA_NUMERO
    JOIN DASE_DE_BATOS.SUCURSALES s ON s.ID = c.SUCURSAL_ID AND s.NOMBRE = m.SUCURSAL_NOMBRE
    JOIN DASE_DE_BATOS.EMPLEADOS e ON e.DNI = m.EMPLEADO_DNI AND e.NOMBRE = m.EMPLEADO_NOMBRE AND e.APELLIDO = m.EMPLEADO_APELLIDO
    JOIN DASE_DE_BATOS.COMPROBANTE_TIPOS ct ON ct.TIPO = m.TICKET_TIPO_COMPROBANTE
    WHERE m.TICKET_NUMERO IS NOT NULL AND m.TICKET_SUBTOTAL_PRODUCTOS IS NOT NULL AND m.TICKET_TOTAL_TICKET IS NOT NULL
...
CREATE PROCEDURE DASE_DE_BATOS.SP_PRODUCTOS AS
	...
	INSERT INTO DASE_DE_BATOS.PRODUCTOS (NOMBRE, DESCRIPCION, SUBCATEGORIA_ID, MARCA_ID)
		...
		(
			...
			WHERE sc.NOMBRE = m.PRODUCTO_SUB_CATEGORIA AND c.NOMBRE = m.PRODUCTO_CATEGORIA
		),
		(
			SELECT TOP 1 ma.ID
...
CREATE PROCEDURE DASE_DE_BATOS.SP_PRODUCTOS_APLICABLE_PROMOCION AS
	...
	INSERT INTO DASE_DE_BATOS.PRODUCTO_APLICABLE_PROMOCION (PRODUCTO_ID, CODIGO_PROMOCION)
		...
		m.PROMO_CODIGO
		FROM gd_esquema.Maestra m
		WHERE m.PRODUCTO_NOMBRE IS NOT NULL AND m.PRODUCTO_DESCRIPCION IS NOT NULL
...
CREATE PROCEDURE DASE_DE_BATOS.SP_ITEMS_VENTA AS
	BEGIN
	INSERT INTO DASE_DE_BATOS.ITEMS_VENTA (VENTA_ID, PRODUCTO_ID, SUBTOTAL, CANTIDAD, TOTAL_PROMOCIONES, TOTAL)
		SELECT DISTINCT
		(
			SELECT TOP 1 v.ID
			...
			WHERE v.NUMERO = m.TICKET_NUMERO AND c.NUMERO = m.CAJA_NUMERO AND s.NOMBRE = m.SUCURSAL_NOMBRE
		),
		(
			SELECT TOP 1 p.ID
			FROM DASE_DE_BATOS.PRODUCTOS p
		...
		FROM gd_esquema.Maestra m
		WHERE m.TICKET_NUMERO IS NOT NULL AND m.CAJA_NUMERO IS NOT NULL AND m.SUCURSAL_NOMBRE IS NOT NULL
		AND m.PRODUCTO_NOMBRE IS NOT NULL AND m.PRODUCTO_DESCRIPCION IS NOT NULL
		AND m.TICKET_DET_PRECIO IS NOT NULL
		AND m.TICKET_DET_CANTIDAD IS NOT NULL AND m.PROMO_APLICADA_DESCUENTO IS NOT NULL AND m.TICKET_DET_TOTAL IS NOT NULL
		GROUP BY m.TICKET_NUMERO, m.CAJA_NUMERO, m.SUCURSAL_NOMBRE, m.PRODUCTO_NOMBRE, m.PRODUCTO_DESCRIPCION, m.TICKET_DET_PRECIO
	END
GO
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        pp.ID
    FROM gd_esquema.Maestra m
	JOIN DASE_DE_BATOS.VENTAS v ON v.NUMERO = m.TICKET_NUMERO
	JOIN DASE_DE_BATOS.CAJAS c ON c.NUMERO = m.CAJA_NUMERO AND c.ID = v.CAJA_ID
	JOIN DASE_DE_BATOS.SUCURSALES s ON s.NOMBRE = m.SUCURSAL_NOMBRE AND s.ID = c.SUCURSAL_ID
	JOIN DASE_DE_BATOS.ITEMS_VENTA iv ON iv.VENTA_ID = v.ID
	JOIN DASE_DE_BATOS.PRODUCTO_APLICABLE_PROMOCION pp ON pp.CODIGO_PROMOCION = m.PROMO_CODIGO
	JOIN DASE_DE_BATOS.PRODUCTOS p ON p.ID = pp.PRODUCTO_ID AND p.NOMBRE = m.PRODUCTO_NOMBRE AND p.DESCRIPCION = m.PRODUCTO_DESCRIPCION
...
CREATE PROCEDURE DASE_DE_BATOS.SP_ENVIOS AS
	BEGIN
    INSERT INTO DASE_DE_BATOS.ENVIOS (FECHA_PROGRAMADA, FECHA_HORA_ENTREGA, HORARIO_INICIO, HORARIO_FIN, COSTO, ESTADO_ENVIO_ID, VENTA_ID, CLIENTE_ID)
		SELECT DISTINCT
		...
		(
			...
			JOIN DASE_DE_BATOS.CAJAS c ON c.ID = v.CAJA_ID
			JOIN DASE_DE_BATOS.SUCURSALES s ON s.ID = c.SUCURSAL_ID
			WHERE v.NUMERO = m.TICKET_NUMERO AND m.SUCURSAL_NOMBRE IS NOT NULL
		),
		(
			SELECT TOP 1 cl.ID
			FROM DASE_DE_BATOS.CLIENTES cl
			WHERE cl.DNI = m.CLIENTE_DNI AND cl.NOMBRE = m.CLIENTE_NOMBRE AND cl.APELLIDO = m.CLIENTE_APELLIDO
		)
		FROM gd_esquema.Maestra m
		WHERE m.ENVIO_FECHA_PROGRAMADA IS NOT NULL AND m.ENVIO_FECHA_ENTREGA IS NOT NULL AND m.ENVIO_HORA_INICIO IS NOT NULL AND m.ENVIO_HORA_FIN IS NOT NULL AND m.ENVIO_COSTO IS NOT NULL
			AND m.TICKET_NUMERO IS NOT NULL AND m.SUCURSAL_NOMBRE IS NOT NULL
			AND m.CLIENTE_DNI IS NOT NULL AND m.CLIENTE_NOMBRE IS NOT NULL AND m.CLIENTE_APELLIDO IS NOT NULL
	END
GO
...
BEGIN
    INSERT INTO DASE_DE_BATOS.PAGOS (FECHA_HORA, IMPORTE, TOTAL_DESCUENTOS, VENTA_ID, MEDIO_DE_PAGO_ID)
    SELECT DISTINCT
        m.PAGO_FECHA,
        m.PAGO_IMPORTE,
    ...
    JOIN DASE_DE_BATOS.CAJAS c ON c.ID = v.CAJA_ID
    JOIN DASE_DE_BATOS.SUCURSALES s ON s.ID = c.SUCURSAL_ID AND s.NOMBRE = m.SUCURSAL_NOMBRE
    JOIN DASE_DE_BATOS.MEDIOS_DE_PAGO mp ON mp.NOMBRE = m.PAGO_MEDIO_PAGO
...
CREATE PROCEDURE DASE_DE_BATOS.SP_DETALLE_PAGO_TARJETA AS
	BEGIN
	INSERT INTO DASE_DE_BATOS.DETALLE_PAGO_TARJETA (PAGO_ID, NRO_TARJETA, FECHA_VENCIMIENTO_TARJETA, CUOTAS)
		SELECT DISTINCT
		p.ID,
		m.PAGO_TARJETA_NRO,
		...
		JOIN DASE_DE_BATOS.CAJAS c ON c.ID = v.CAJA_ID
		JOIN DASE_DE_BATOS.SUCURSALES s ON s.ID = c.SUCURSAL_ID
		JOIN DASE_DE_BATOS.PAGOS p ON p.FECHA_HORA = m.PAGO_FECHA AND p.IMPORTE = m.PAGO_IMPORTE AND p.VENTA_ID = v.ID
		WHERE s.NOMBRE = m.SUCURSAL_NOMBRE
			AND m.PAGO_FECHA IS NOT NULL AND m.PAGO_IMPORTE IS NOT NULL AND m.TICKET_NUMERO IS NOT NULL AND m.PAGO_TIPO_MEDIO_PAGO IS NOT NULL
			AND m.PAGO_TARJETA_NRO IS NOT NULL AND m.PAGO_TARJETA_FECHA_VENC IS NOT NULL AND m.PAGO_TARJETA_CUOTAS IS NOT NULL
	END
GO
...
CREATE PROCEDURE DASE_DE_BATOS.SP_PAGO_DESCUENTO AS
    ...
    INSERT INTO DASE_DE_BATOS.PAGO_DESCUENTO (DESCUENTO_ID, PAGO_ID)
		...
		(
			...
			WHERE p.FECHA_HORA = m.PAGO_FECHA AND p.IMPORTE = m.PAGO_IMPORTE AND p.TOTAL_DESCUENTOS = m.PAGO_DESCUENTO_APLICADO
		) AS PAGO_ID
		FROM gd_esquema.Maestra m
			WHERE m.DESCUENTO_DESCRIPCION IS NOT NULL AND m.DESCUENTO_FECHA_INICIO IS NOT NULL AND m.DESCUENTO_FECHA_FIN IS NOT NULL AND m.DESCUENTO_PORCENTAJE_DESC IS NOT NULL AND m.DESCUENTO_TOPE IS NOT NULL
		AND m.PAGO_FECHA IS NOT NULL AND m.PAGO_IMPORTE IS NOT NULL AND m.PAGO_MEDIO_PAGO IS NOT NULL
	END
GO
----
