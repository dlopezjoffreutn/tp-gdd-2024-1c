...
CREATE PROCEDURE DASE_DE_BATOS.SP_ITEMS_VENTA AS
	BEGIN
	INSERT INTO DASE_DE_BATOS.ITEMS_VENTA (VENTA_ID, PRODUCTO_ID, PRECIO, CANTIDAD)
		SELECT DISTINCT
		(
			SELECT TOP 1 v.ID
		...
		GROUP BY m.TICKET_NUMERO, m.CAJA_NUMERO, m.SUCURSAL_NOMBRE, m.PRODUCTO_NOMBRE, m.PRODUCTO_DESCRIPCION
	END
GO
...
BEGIN
    INSERT INTO DASE_DE_BATOS.PAGOS (FECHA_HORA, IMPORTE, VENTA_ID, MEDIO_DE_PAGO_ID)
    SELECT DISTINCT
        m.PAGO_FECHA,
        m.PAGO_IMPORTE,
        v.ID AS VENTA_ID,
...
CREATE PROCEDURE DASE_DE_BATOS.SP_PAGO_DESCUENTO AS
    ...
    INSERT INTO DASE_DE_BATOS.PAGO_DESCUENTO (DESCUENTO_ID, PAGO_ID)
		...
		(
			...
			WHERE p.FECHA_HORA = m.PAGO_FECHA AND p.IMPORTE = m.PAGO_IMPORTE
		) AS PAGO_ID
		FROM gd_esquema.Maestra m
				WHERE m.DESCUENTO_DESCRIPCION IS NOT NULL AND m.DESCUENTO_FECHA_INICIO IS NOT NULL AND m.DESCUENTO_FECHA_FIN IS NOT NULL AND m.DESCUENTO_PORCENTAJE_DESC IS NOT NULL AND m.DESCUENTO_TOPE IS NOT NULL
